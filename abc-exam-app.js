...
const CASESTUDY_QUIZZES = {
  ...
  CS3: [{
    ...
    answer: 2,
    isCalc: true,
    explanation: "トータルリターン = (売却益 + 分配金) ÷ 取得単価 = (9,600 - 8,000 + 1,000) ÷ 8,000 = 2,600 ÷ 8,000 = 32.5%。※選択肢「30%」は近似値として最も近い。",
    keyword: "トータルリターンの計算"
  }],
  CS4: [{
    id: "cs4-1",
  ...
  CS5: [{
    ...
    id: "cs5-3",
    q: "クーポン（表面利率）3%・額面100円の社債が98円で発行された場合、発行時の利回りはおよそいくらか（簡易計算）？",
    choices: ["約3.0%", "約3.1%", "約4.1%", "約5.0%"],
    answer: 1,
    isCalc: true,
    explanation: "簡易利回り ≈ (クーポン + 発行差益/残存年数) ÷ 発行価格。発行差益2円・残存10年の場合 ≈ (3 + 0.2) ÷ 98 ≈ 3.27%。簡易的には「クーポン > 発行価格を下回れば利回り > クーポン率」と理解する。",
    keyword: "社債の発行条件と利回り"
  }, {
    id: "cs5-4",
  ...
  CS6: [{
    ...
    id: "cs6-2",
    q: "標準偏差11.27%の証券Xと8.33%の証券Yを8:2で組み合わせたポートフォリオのリスクは、単純加重平均（10.68%）と比較してどうなるか？",
    choices: ["10.68%になる（加重平均と一致）", "10.68%より大きくなる", "10.68%より小さくなる（7.35%程度）", "必ずゼロになる"],
    answer: 2,
    isCalc: true,
    explanation: "ポートフォリオ効果により実際の標準偏差は加重平均より小さくなる（例：約7.35%）。2資産の相関が完全に+1でない限り、分散効果によりリスクは単純加重平均を下回る。",
  ...
  CS7: [{
    ...
    id: "cs7-3",
    q: "ファンドAのシャープレシオ：ポートフォリオリターン12%、無リスク金利1%、標準偏差15%。シャープレシオはいくらか？",
...
function CaseStudyTab({
  ...
  }), "\u5B8C\u4E86\u6E08\u307F \u2014 \u518D\u6311\u6226\u3059\u308B") : `${CASESTUDY_SECTIONS.find(s => s.id === section)?.label} テスト（${quizzes.length}問）を開始`), state[quizKey] && /*#__PURE__*/React.createElement(QuizComponent, {
    quizzes: quizzes,
    tabId: "casestudy",
    sectionId: section,
    accentColor: color,
...
}, ...Object.entries(CASESTUDY_QUIZZES).map(([sec, qs]) => ({
  tab: "casestudy",
  sec,
  quizzes: qs.filter(q => q.isCalc)
}))];
